// -*- C++ -*-
//
// Package:    DQM/GEMLocal
// Class:      GEMLocal
//
/**\class GEMLocal GEMLocal.cc DQM/GEMLocal/plugins/GEMLocal.cc
...
class GEMLocal : public DQMEDAnalyzer {
public:
...
void GEMLocal::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  // using namespace edm;
...
void GEMLocal::bookHistograms(DQMStore::IBooker& ibook, edm::Run const& run, edm::EventSetup const& iSetup) {
  edm::ESHandle<GEMGeometry> hGEMGeom;
  iSetup.get<MuonGeometryRecord>().get(hGEMGeom);
  const GEMGeometry* GEMGeometry_ = &*hGEMGeom;
...
void GEMLocal::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add("GEMLocal", desc);
}
